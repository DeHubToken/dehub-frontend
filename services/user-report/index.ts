...
export async function blockDM(data: { conversationId: string; reason: string; address: string }) {
  const url = `/dm/block`; // API endpoint for blocking a DM
  try {
    ...
    console.error("Error blocking DM:", error);
...
export async function unBlockUser(data: { conversationId: string; address: string }) {
  const url = `/dm/un-block/${data.conversationId}?address=${data.address}`; // API endpoint for unblocking a DM
  try {
