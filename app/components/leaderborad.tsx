...
export async function Leaderboard() {
  ...
  return (
    <div className="sticky right-0 top-0 hidden h-screen w-full items-start justify-start overflow-hidden px-4 pb-0 pt-20 md:flex">
      <div className="size-full">
        <div className="flex size-auto items-center justify-between">
          <h1 className="font-tanker text-2xl">Leaderboard</h1>
          <LeaderBoardModal
            data={res.data}
            trigger={
        ...
        <div className="relative size-full overflow-x-hidden overflow-y-scroll pb-40 pt-4">
          ...
          <div className="flex h-auto w-full flex-col items-start justify-start">
            {res.data.result.byWalletBalance.map((user, index) => (
              <div
                key={index}
                className="flex h-auto w-full items-center justify-between gap-4 border-b-2 border-theme-mine-shaft-dark py-4 dark:border-theme-mine-shaft"
              >
                <div className="flex size-auto items-center justify-start gap-4">
                ...
                <p className="text-sm font-medium">{formatNumber(user.total)}</p>
              </div>
            ))}
          </div>
        </div>
